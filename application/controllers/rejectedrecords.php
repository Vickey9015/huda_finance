...
// error_reporting(0);
class Rejectedrecords extends CI_Controller {
    ...
    function __construct() {
		...
		$this->load->model('rejectedrecords_model');
        $this->load->helper('url');
        $this->load->library('session');
	...
	function RejectedRecordList(){
		...
		if(in_array($this->role_id, ['3','4','5','9'], TRUE)){
			...
			$this->load->view('rejectedrecordlist_view',$data);
		}else{
			redirect(base_url());
		}
	...
	function GetUpdatedRecordList(){
        ...
        //  echo 'hlw'; die();
        //$session_data = $this->session->all_userdata();
        $userData = $this->rejectedrecords_model->get_datatables();
        ...
        $output = array(
            "recordsTotal" => $this->rejectedrecords_model->count_all(),
            "recordsFiltered" => $this->rejectedrecords_model->count_filtered(),
            "data" => $data
        );
