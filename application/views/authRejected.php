...
?>
 <div class="col-md-12 inner_white mt-4">
        <div class="row">
          <div class="pink_bar"> <h4>Rejected List</h4> <h4>Total Amount: &#x20B9; <span id="totalAmount"></span></h4><div class="col-md-6 ml-auto pink_right"> 

</div>  </div>
        </div>
		 <form id="link_pay_forms" method="POST" enctype="multipart/form-data" action="<?php echo base_url() ?>authorizerRejected/authRejectedList">
		     <?php echo form_open();
...
$path      =base_url(). '/upload/'.$annexure_file[$item["annexure_type"]].'/'.$item["file_name"];
					...
					$totalAmount            +=$item["rejected_by_LAO_sum"]+$item["rejected_by_releaser_sum"];
                                   ?>
  <tr>
...
<td><a href="complexDownload" download="<?php echo  $item["file_name"]; ?>" onclick="this.href='<?php echo $path; ?>';"><?php echo  htmlspecialchars($item["file_name"]); ?></a></td>
           <td><?php	echo 	htmlspecialchars($annexure_type[$item["annexure_type"]]);	?></td>
            <td data-order="<?php echo date('d-m-Y',strtotime($item['created_on'])); ?>"><?php echo htmlspecialchars(date('d-m-Y',strtotime($item['created_on'])));  ?>  </td>
            <td data-order="<?php echo date('d-m-Y',strtotime($item['rejected_on'])); ?>"><?php echo htmlspecialchars(date('d-m-Y',strtotime($item['rejected_on'])));  ?>  </td>
	...
	<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>/4"><?php  echo htmlspecialchars($item["rejected_by_LAO"]);  ?></a><br> &#x20B9; <?php  if($item["rejected_by_LAO_sum"] > 0){
				...
				echo '0';
												...
												<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>/5"><?php  echo $item["rejected_by_releaser"];  ?></a><br> &#x20B9; <?php  if($item["rejected_by_releaser_sum"] > 0){
				echo moneyFormatIndia(htmlspecialchars($item["rejected_by_releaser_sum"]));
				}else{
				echo '0';
				}    ?></td>		
	<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>/4"><?php  echo htmlspecialchars($item["reason"]);  ?></a></td>   												
