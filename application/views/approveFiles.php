 ...
 ?>
  <div class="col-md-12 inner_white mt-4 dash_x">
      ...
      <div class="alert-success fade in" id="successMsg"> </div>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
               <?php if($role_id == 3){ ?>
                <h5 class="modal-title" id="exampleModalLabel">SUBMIT TO LAO</h5>
		...
		<form id="link_pay_forms" method="POST" enctype="multipart/form-data" action="<?php echo base_url() ?>FileApprove/approveFiles">
		    <?php echo form_open(); ?>
...
<div class="table-responsive" id="table_data">
 ...
 <!--  <?php if($role_id == 4){ ?>
  ...
  SO Maker name
      ...
      <th scope="col">
        <!-- <input type="checkbox" class="checkboxAll" name="selectAll" id="selectAllDomainList" /> -->
        Select
      </th>
...
foreach ($result as $key => $value) {
  // print_r($value["id"]);
    ...
    <tr>
    <td><input type="checkbox" class="checkbox1" name="id[]" id="idCheckbox" value="<?php echo $value["id"].','.$value["total_amount"].','.$value["reference_number"];?>"
    <?php 
    if($role_id==3)
    {
      if($value["is_error"]!=3)
    ...
    ?>
    >
    <!-- <span class="checkmark"><?php echo $value["id"];?></span> -->
    </td>
    <td><?php	echo htmlspecialchars($value["file_name"]);	?></td>
    <td>
      <a href="<?php echo base_url() ?>FileApprove/getRecords/<?php echo htmlspecialchars($value['reference_number']);?>">
      <?php	echo 	htmlspecialchars($value["total_record"]);	?>
      </a>
    </td>
    <td><?php	echo htmlspecialchars($value["reference_number"]);	?></td>
    <td><?php	echo htmlspecialchars($value["total_amount"]);	?></td>

    <td>
    <?php if($value["is_error"]==1){
          echo "<span style='color:#00008B'><b>Pending Validation</b</span>";
        }else if($value["is_error"]==2){
          echo "<span style='color:#EA4335'><b>Validation Failed</b</span>";
        }else if($value["is_error"]==3){
          echo "<span style='color:#00008B'><b>Pending for Initiation</b</span>";
        }else if($value["is_error"]==4){
          echo "<span style='color:#28A74B'><b>Pending at LAO</b</span>";
        }else if($value["is_error"]==5){
          echo "<span style='color:#28A74B'><b>Approve</b</span>";
        }
        ?></td>
    ...
    <?php } ?>
        ...
        if($role_id == 3){ ?>
        <br>
        <button type="button" value="Approve" class="btn upload_button" disabled>SUBMIT TO LAO</button>
        <?php }elseif($role_id == 4){?>
          <br>
          <button type="button" value="ApproveLAO" class="btn upload_button" disabled>APPROVE</button>
        <?php } ?>
        </form>
      </div>
	</div>	
...
"language": {
    ...
    $("#verifyOTP").click(function(){
      ...
      var usertype = $(".upload_button").val();
