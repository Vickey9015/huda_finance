...
?>
          <div class="pink_bar"> <h4>Reinitiated List of Annexures</h4> <h4>Total Amount: &#x20B9; <span id="totalAmount"></span></h4><div class="col-md-6 ml-auto pink_right"> 
</div>  </div>
        </div>
		 <form id="link_pay_forms" method="POST" enctype="multipart/form-data" action="<?php echo base_url() ?>mandatesList/reInitiated">
		     <?php echo form_open();
...
$path      =base_url(). '/upload/'.$annexure_file[$item["annexure_type"]].'/'.$item["file_name"];
				...
				$totalAmount            +=$item["reinitiated_sum"];
                                   ?>
  <tr>
...
<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo htmlspecialchars($ref_no);?>/<?php echo htmlspecialchars($type);?>/10"><?php  echo $item["reinitiated"]; ?></a> <br> &#x20B9; <?php  if($item["reinitiated_sum"] > 0){
				echo moneyFormatIndia(htmlspecialchars($item["reinitiated_sum"]));
				}else{
				echo '0';
				}  ?></td>										
