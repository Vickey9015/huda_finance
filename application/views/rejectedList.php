...
?>
          <div class="pink_bar"> <h4>Rejected List of Annexures</h4> <h4>Total Amount: &#x20B9; <span id="totalAmount"></span></h4><div class="col-md-6 ml-auto pink_right"> 
</div>  </div>
        </div>
		 <form id="link_pay_forms" method="POST" enctype="multipart/form-data" action="<?php echo base_url() ?>rejectedList/RejectedList">
		     <?php echo form_open();
...
foreach($result as $key=>$item){
    $dateOrder = date('Y-m-d', strtotime($item['rejected_on'])); 
if($item["rejected_by_LAO"] > 0 or $item["rejected_by_releaser"] > 0){					
...
<td><a href="complexDownload" download="<?php echo  $item["file_name"]; ?>" onclick="this.href='<?php echo $path; ?>';"><?php echo  $item["file_name"]; ?></a></td>
            ...
            <td data-order="<?php echo $dateOrder; ?>"><?php echo htmlspecialchars(date('d M Y',strtotime($item['rejected_on'])));?>  
            </td>
