...
<div class="col-md-12 inner_white mt-4 dash_x pink_bar">
  ...
  <div class="col-md-12 alignCenterPhone">
        ...
        <?php 
        $total_file_name = "report/Unclaimed/Total/UnclaimedPayment_".date('Y-m-d').".xlsx";
        $total_file_link= base_url()."report/Unclaimed/Total/UnclaimedPayment_".date('Y-m-d').".xlsx"; 
        if(file_exists($total_file_name)){ ?>
      
            <button type="submit" class="" onclick="window.location.href='<?php echo $total_file_link; ?>'">Total Records</button>
        <?php }else{ ?>
          
            <button type="submit" class="" disabled>Total Records</button>

        <?php } ?>

          <?php 
          
          foreach($this->session->userdata('zones_option') as $zone){ 
                      $zone_id= $zone['id']; 
                      $zone_name=$zone['name'];
                    // echo $file_link= base_url()."report/Unclaimed/UnclaimedPayment_".date('Y-m-d')."_".$zone_id.".xlsx"; 
                    $filename = "report/Unclaimed/Total/UnclaimedPayment_".date('Y-m-d')."_".$zone_id.".xlsx";
                    $file_link= base_url()."report/Unclaimed/Total/UnclaimedPayment_".date('Y-m-d')."_".$zone_id.".xlsx"; 
  ...
  function doSomething() {
    ...
    table = $('#example1').DataTable({
      ...
      "ajax": {
        ...
        "url": "<?php echo base_url('SuccessRecords/GetTotalRecordList') ?>",
        "type": "POST",
        data: {
