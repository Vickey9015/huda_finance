...
<?php if($this->session->flashdata('msg')): ?>
    ...
    <strong>Success! </strong><?php echo htmlspecialchars($this->session->flashdata('msg')); ?>
	</div>
<?php endif; ?>
          <div class="pink_bar"> <h4>In Process To Releaser</h4> <h4>Total Amount: &#x20B9; <span id="totalAmount"></span></h4><div class="col-md-6 ml-auto pink_right"> 

</div>  </div>
        </div>
		<form id="link_pay_forms" method="POST"  enctype="multipart/form-data" action="<?php echo base_url() ?>approvallist/approvalWaiting" style="display:none">
		    <?php echo form_open();
...
<div class="form-group input-group loc">
      ...
      <select class="form-control input-sm" id="annexure_type" name="annexure_type" style="height:40px;">
                                 ...
                                 <option value="2" <?php if ($_REQUEST['annexure_type'] == '2') { echo 'selected'; } ?>>Lower Court &nbsp;</option>
 								 ...
 								 <option value="5" <?php if ($_REQUEST['annexure_type'] == '5') { echo 'selected'; } ?>>DD &nbsp;</option>										            </select>
                                                </div>
                                            </div>
                                        </div>
...
<div class="table-responsive" id="table_data">
  ...
  Annexure type
      ...
      <th scope="col">In process to Releaser</th>
      ...
      <input type="hidden" id="btn-action_file_auth" name="action" value="">
	  <input type="hidden" id="current_status" name="current_status" value="8">
...
foreach($result as $key=>$item){
	if($item["in_process_to_releaser"] != 0){
...
$path      =base_url(). '/upload/'.$annexure_file[$item["annexure_type"]].'/'.$item["file_name"];
                                   ...
                                   $totalAmount            +=$item["in_process_to_releaser_sum"];
                                   ?>
  <tr>
  <?php if ($role_id == 4) {?>
     <td scope="row"> 
...
<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>/8"><?php echo  htmlspecialchars($item["in_process_to_releaser"]); ?></a> <br> &#x20B9;  <span class="netAmount"><?php  if($item["in_process_to_releaser_sum"] > 0){
...
<?php if ($role_id == 4) {?>
<button type="button"  disabled="true" ng-click="confirmRelease($event,'reject','file','auth')" class="btn upload_button mr-4" name="action" value="reject" style="background-color:#353535;margin-left: 35%;">Reject</button>
<button type="button"  disabled="true"  ng-click="confirmImmediate($event,'immediate','file')" class="btn upload_button">
  Send to Releaser Immediately 
</button>
<?php } ?>
</form>
</div>
