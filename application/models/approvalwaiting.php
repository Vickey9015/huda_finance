...
?>
          <div class="pink_bar"> <h4>Waiting For Approval</h4> <h4>Total Amount: &#x20B9; <span id="totalAmount"></span></h4><div class="col-md-6 ml-auto pink_right"> 

</div>  </div>
        </div>
        <?php echo validation_errors(); ?>
		<form id="link_pay_forms" method="POST" enctype="multipart/form-data" action="<?php echo base_url() ?>approvallist/approvalWaiting">
		    <?php echo form_open(); ?>
...
<div class="table-responsive" id="table_data">
   <form action="<?php echo base_url() ?>approvallist/changeStatus" name="releaseForm" method="POST" class="form-horizontal" enctype="multipart/form-data">
       <?php echo form_open(); ?>
...
$path      =base_url(). '/upload/'.$annexure_file[$item["annexure_type"]].'/'.$item["file_name"];
                                   ...
                                   $totalAmount            +=$item["with_checker_sum"];
                                   ?>
  <tr>
  <?php if($role_id == 4){ ?>
     <td scope="row"> 
     <label class="custom_chk">
   <input type="checkbox" class="checkbox1" name="reference_number[]" id="color" value="<?php echo $item["reference_number"];?>">
  <span class="checkmark"></span>
</label>
      </td>
      <?php } ?>
      <td><!--<a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>">--><?php echo  $item["maker_name"]; ?><!--</a>--></td>
      <td><a href="complexDownload" download="<?php echo  $item["file_name"]; ?>" onclick="this.href='<?php echo $path; ?>';"><?php echo  $item["file_name"]; ?></a></td>
      <td><?php	echo 	$annexure_type[$item["annexure_type"]];	?></td>
      <td><?php echo date('d-m-Y', strtotime($item["created_on"])); ?></td>
	  <td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>"><?php  echo $item["totalRecord"];  ?></a><br> &#x20B9; <?php  if($item["totalRecord_sum"] > 0){
				echo moneyFormatIndia($item["totalRecord_sum"]);
				}else{
				echo '0';
				}  ?></td>
<td><a href="<?php echo base_url() ?>innerList/InnerMandateList/<?php echo $ref_no;?>/<?php echo $type;?>/2"><?php echo  $item["with_checker"]; ?></a> <br> &#x20B9; <span class="netAmount"><?php  if($item["with_checker_sum"] > 0){
				echo moneyFormatIndia($item["with_checker_sum"]);
				}else{
				echo '0';
				}  ?></span></td>
	</tr>
...
<?php if($role_id == 4){ ?>
<button type="button" disabled="true" ng-click="confirmRelease($event,'reject','file','auth')" value="reject" class="btn upload_button mr-4" style="background-color:#353535;margin-left: 35%;">Reject</button>
<button type="button" disabled="true" ng-click="confirmRelease($event,'authorization','file','auth')" value="rsubmit" class="btn upload_button">
  Submit to releaser
</button>
<?php } ?>
</form>
	...
	$(window).load(function ()
{
	...
	jQuery('#totalAmount').html('<?php echo $amount; ?>');
